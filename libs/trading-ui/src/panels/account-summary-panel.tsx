/**
 * File:        libs/trading-ui/src/panels/account-summary-panel.tsx
 * Module:      trading-ui · Panels
 * Purpose:     Account stats, margin meter, and 30d P&L sparkline.
 *
 * Exports:
 *   - AccountSummaryPanel({ snapshot? }) → ReactNode   — uses ACCOUNT mock when snapshot is absent
 *
 * Depends on:
 *   - ../types/instrument — AccountSnapshot
 *   - ../lib/format-utils — fmt, pnlClass, pnlSign
 *   - ../lib/mock-data — ACCOUNT, P_AND_L_HISTORY
 *
 * Side-effects:
 *   - none
 *
 * Key invariants:
 *   - snapshot prop is optional; falls back to ACCOUNT mock for dev/demo mode
 *   - marginPct is clamped to 100 to prevent bar overflow
 *
 * Read order:
 *   1. AccountSummaryPanel — component entry
 *   2. PnLSparkline — SVG sparkline helper
 *
 * Author:      BharatERP
