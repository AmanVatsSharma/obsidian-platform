/**
 * File:        libs/trading-ui/src/panels/bottom-tabs-panel.tsx
 * Module:      trading-ui · Panels
 * Purpose:     Bottom tab strip — Positions, Orders, History, Calendar, News.
 *
 * Exports:
 *   - BottomTabsPanel({ positions, onClosePosition, pendingOrders? }) → ReactNode
 *
 * Depends on:
 *   - ../types/instrument — OpenPosition, PendingOrder
 *   - ../lib/format-utils — fmt, pnlClass, pnlSign
 *   - ../lib/mock-data — ECONOMIC_CALENDAR, NEWS, PENDING_ORDERS, TRADE_HISTORY
 *   - lucide-react — tab icons
 *
 * Side-effects:
 *   - none
 *
 * Key invariants:
 *   - pendingOrders defaults to PENDING_ORDERS mock when not supplied
 *   - PositionsTable, OrdersTable, etc. are internal sub-components; not exported
 *
 * Read order:
 *   1. BottomTabsPanel — outer tab container
 *   2. PositionsTable — primary active panel
 *
 * Author:      BharatERP
...
import type { OpenPosition, PendingOrder } from '../types/instrument';
  ...
  return (
    <div className="bottom-content">
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '6px 16px', background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border)' }}>
        ...
        <button type="button" style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 8px', borderRadius: '4px', background: 'var(--bear-dim)', border: '1px solid var(--bear-dim)', color: 'var(--bear)', fontSize: '10px', fontWeight: 600, cursor: 'pointer' }}>
          Close All
        </button>
      </div>
      <table className="data-table">
        <thead>
          <tr>
            <th>Symbol</th><th>Type</th><th>Lots</th><th>Open Price</th>
            <th>Current</th><th>SL</th><th>TP</th><th>Swap</th><th>P&L</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {positions.map((p) => (
            <tr key={p.id}>
              ...
              <td>
                <div className="td-actions">
                  <button type="button" className="td-action-btn" aria-label="Edit position"><Edit size={10} /></button>
                  <button type="button" className="td-action-btn close" onClick={() => onClose(p.id)}>Close</button>
                </div>
              </td>
            </tr>
          ))}
  ...
  return (
    <div className="bottom-content">
      <table className="data-table">
        <thead>
          <tr>
            <th>Symbol</th><th>Type</th><th>Lots</th><th>Price</th>
            <th>Distance</th><th>SL</th><th>TP</th><th>Expiry</th><th>Created</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {orders.map((o) => {
            ...
            return (
              <tr key={o.id}>
                ...
                <td>
                  <div className="td-actions">
                    <button type="button" className="td-action-btn" aria-label="Edit order"><Edit size={10} /></button>
                    <button type="button" className="td-action-btn close">Cancel</button>
                  </div>
                </td>
              </tr>
            );
  ...
  return (
    <div className="bottom-content">
      ...
      <table className="data-table">
        <thead>
          <tr>
            <th>Symbol</th><th>Type</th><th>Lots</th><th>Open</th>
            <th>Close</th><th>Open Time</th><th>Close Time</th><th>Duration</th><th>P&L</th>
          </tr>
        </thead>
        <tbody>
          {TRADE_HISTORY.map((t) => (
    ...
    <div className="bottom-content">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '60px 28px 28px 1fr 80px 80px 80px', gap: '0 8px', padding: '4px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
        {['Time', '', 'Impact', 'Event', 'Forecast', 'Previous', 'Actual'].map((h) => (
          <span key={h || 'flag'} style={{ fontSize: '9px', color: 'var(--text-muted)', fontFamily: 'var(--font-data)', letterSpacing: '0.06em', textTransform: 'uppercase', textAlign: h === 'Forecast' || h === 'Previous' || h === 'Actual' ? 'right' : 'left' }}>{h}</span>
        ))}
      </div>
      {filtered.map((e) => (
        <div key={e.id} className="cal-item">
          ...
          <span className={`cal-val ${
            e.actual != null && e.forecast != null && !Number.isNaN(parseFloat(String(e.actual))) && !Number.isNaN(parseFloat(String(e.forecast)))
              ? parseFloat(String(e.actual)) > parseFloat(String(e.forecast)) ? 'actual-up' : 'actual-down'
              : ''
          }`}>{e.actual ?? '—'}</span>
        </div>
      ))}
