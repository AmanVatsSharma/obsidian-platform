/**
 * File:        libs/trading-ui/src/panels/chart-panel.tsx
 * Module:      trading-ui · Panels
 * Purpose:     Candlestick + volume chart using TradingView lightweight-charts v4 API.
 *
 * Exports:
 *   - ChartPanel({ instrument, prices }) → ReactNode
 *
 * Depends on:
 *   - lightweight-charts — createChart, CrosshairMode (dynamic import for SSR safety)
 *   - ../types/instrument — Instrument
 *   - ../lib/format-utils — fmtPrice, pnlSign
 *   - ../lib/mock-data — TIMEFRAMES, generateOHLCV
 *   - lucide-react — toolbar icons
 *
 * Side-effects:
 *   - Creates / destroys a lightweight-charts instance on instrument change
 *   - setInterval tick simulation (1500ms) updates the latest candle
 *   - ResizeObserver adjusts chart dimensions on container resize
 *
 * Key invariants:
 *   - lightweight-charts is imported dynamically to avoid SSR failures in Next.js
 *   - candleRef and volRef are imperatively managed; React state is only used for toolbar/overlay
 *   - Chart is destroyed and recreated when instrument.symbol changes (not on every tick)
 *
 * Read order:
 *   1. ChartPanel component — outer shell and toolbar
 *   2. useEffect (init) — chart lifecycle
 *   3. useEffect (tick) — live price update
 *
 * Author:      BharatERP
