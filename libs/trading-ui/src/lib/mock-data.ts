/**
 * File:        libs/trading-ui/src/lib/mock-data.ts
 * Module:      trading-ui · Mock Data
 * Purpose:     Typed seed catalogue of instruments, positions, orders, and account state for demo/dev mode.
 *
 * Exports:
 *   - generateOHLCV(basePrice, count?, volatility?) → CandleRow[]  — synthetic candlestick generator
 *   - DOM_DATA(basePrice) → DomLevel[]               — synthetic depth-of-market levels
 *   - INSTRUMENTS                                    — 19-instrument mock catalogue (forex/crypto/indices/commodities)
 *   - OPEN_POSITIONS                                 — 6 mock open positions
 *   - PENDING_ORDERS                                 — 4 mock resting orders
 *   - TRADE_HISTORY                                  — 8 mock closed trades
 *   - ECONOMIC_CALENDAR                              — 8 mock calendar events
 *   - NEWS                                           — 8 mock news items
 *   - ACCOUNT                                        — single mock account snapshot
 *   - TIMEFRAMES                                     — ordered timeframe labels
 *   - P_AND_L_HISTORY                                — 30-day P&L sparkline series
 *
 * Depends on:
 *   - ./instrument (types only)
 *
 * Side-effects:
 *   - none
 *
 * Key invariants:
 *   - All mock data is intentionally static for stable Storybook/test renders;
 *     DOM_DATA and generateOHLCV use Math.random() and are NOT deterministic
 *
 * Read order:
 *   1. INSTRUMENTS — canonical symbol catalogue
 *   2. ACCOUNT — account context used by AccountSummaryPanel
 *
 * Author:      BharatERP
