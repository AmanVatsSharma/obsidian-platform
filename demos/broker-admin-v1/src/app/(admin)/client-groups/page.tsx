 ...
 *
 * Depends on:
 *   - @/lib/mock-data-context — useBrokerData() for clientGroups
 *
 * Side-effects:
...
export default function ClientGroupsPage() {
  const { clientGroups: initial, clients } = useBrokerData();
  const [groups, setGroups] = useState<ClientGroup[]>([...initial]);
  ...
  const totalClients = groups.reduce((s, g) => s + g.clientCount, 0);
    ...
    <div className="flex flex-col">
      <div className="module-header">
        <div>
          ...
          <p className="module-subtitle">{groups.length} groups · {totalClients.toLocaleString()} clients total</p>
        </div>
          ...
          {groups.map(g => (
            <div key={g.id} className="card overflow-hidden" style={{ borderLeft: `3px solid ${g.color}` }}>
              <div className="card-header">
                ...
                <button className="btn-ghost btn btn-xs" onClick={() => setEditing(g)}>
                  <Edit2 size={11} /> Edit
                </button>
              </div>
              <div className="p-4">
